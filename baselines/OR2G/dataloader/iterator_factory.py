...
def get_spatem_rel(data_root,
    ...
    """
    train = spatem_rel_FeatureIter(visual_feature_prefix=os.path.join(data_root, 'object_features'),
                        ...
                        relation_visual_prefix=os.path.join(data_root, 'relation_graph', 'pair_features'),
                        spa_adjacent_matrix_prefix=os.path.join(data_root, 'adjacent', 'spa_adj'),
                        tem_adjacent_matrix_prefix=os.path.join(data_root, 'adjacent', 'tem_adj'),
                        rel_adjacent_matrix_prefix=os.path.join(data_root, 'adjacent', 'rel_adj'),
                        dataset_level_list=os.path.join(data_root, 'dataset-level-train.txt'),
                        dataset_level_relation_list_1=os.path.join(data_root, 'relation_graph', 'relation-semantic_1.txt'),
                        video_level_object_list=os.path.join(data_root, 'video-level-object/'),
                        video_level_relation_list=os.path.join(data_root, 'video-level-relation-A-O/'),
                        name='train',
                        )


    val   = spatem_rel_FeatureIter(visual_feature_prefix=os.path.join(data_root, 'object_features'),
                        spatial_feature_prefix=os.path.join(data_root, 'relation_graph', 'person_object_spatial_feature'),
                        semantic_feature_prefix=os.path.join(data_root, 'object_features_semantic'),
                        relation_semantic_prefix=os.path.join(data_root, 'relation_graph', 'relation_semantic_feature'),
                        relation_visual_prefix=os.path.join(data_root, 'relation_graph', 'pair_features'),
                        ...
                        tem_adjacent_matrix_prefix=os.path.join(data_root, 'adjacent', 'tem_adj'),
                        rel_adjacent_matrix_prefix=os.path.join(data_root, 'adjacent', 'rel_adj'),
                        dataset_level_list=os.path.join(data_root, 'dataset-level-test.txt'),
                        dataset_level_relation_list_1=os.path.join(data_root, 'relation_graph', 'relation-semantic_1.txt'),
                        video_level_object_list=os.path.join(data_root, 'video-level-object/'),
                        video_level_relation_list=os.path.join(data_root, 'video-level-relation-A-O/'),
                        name='test',
