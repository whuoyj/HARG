...
class Config(object):
    ...
    class attributes. 
    """
    def __init__(self, load=True, cfg_dict=None, cfg_level=None, cfg_file=None):
        """
        Args: 
    ...
    def _initialize_cfg(self):
        ...
        if self.need_initialization:
            ...
            if os.path.exists('./configs/pool/base.yaml'):
                ...
                with open("/home/ouyangjun/workspace/data/a/shiXiao/code/Masked-Action-Recognition-main/configs/pool/base.yaml", 'r') as f:
                    cfg = yaml.load(f.read(), Loader=yaml.SafeLoader)
        return cfg
    ...
    def _merge_cfg_from_command(self, args, cfg):
        ...
        for idx, key in enumerate(keys):
            ...
            def get_cmd_val(val_in_cfg, val_from_cmd):
                if type(val_in_cfg) is bool:
                    if 'True' or 'true' in val_from_cmd:
                        return True
                    elif 'False' or 'false' in val_from_cmd:
                        return False
                    else:
                        raise ValueError(f"\"{val_from_cmd}\" is not a bool type!")
                elif type(val_in_cfg) is str:
                    return val_from_cmd
                elif type(val_in_cfg) is float:
                    return float(val_from_cmd)
                elif type(val_in_cfg) is int:
                    return int(val_from_cmd)
                elif type(val_in_cfg) is list:
                    raise ValueError(f"Not support list from command!")

            if len(key_split) == 1:
                cfg[key_split[0]] = get_cmd_val(cfg[key_split[0]], vals[idx])
            elif len(key_split) == 2:
                cfg[key_split[0]][key_split[1]] = get_cmd_val(cfg[key_split[0]][key_split[1]], vals[idx])
            elif len(key_split) == 3:
                cfg[key_split[0]][key_split[1]][key_split[2]] = get_cmd_val(cfg[key_split[0]][key_split[1]][key_split[2]], vals[idx])
            elif len(key_split) == 4:
                cfg[key_split[0]][key_split[1]][key_split[2]][key_split[3]] = get_cmd_val(cfg[key_split[0]][key_split[1]][key_split[2]][key_split[3]], vals[idx])
    ...
    def dump(self):
        def recur(elem):
            if type(elem) is Config:
                out_dict = {}
                for k, c in elem.__dict__.items():
                    out_dict[k] = recur(c)
                return out_dict
            elif type(elem) is argparse.Namespace:
                return elem.__dict__
            else:
                return elem
        out_dict = dict((key, recur(elem)) for key, elem in self.__dict__.items())
