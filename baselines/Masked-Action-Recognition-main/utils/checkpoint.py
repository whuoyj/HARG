...
def is_checkpoint_epoch(cfg, cur_epoch):
    ...
    """
    if cur_epoch + 1 >= cfg.OPTIMIZER.MAX_EPOCH - 10 and cfg.PRETRAIN.ENABLE is False and not cfg.MM_RETRIEVAL.ENABLE:
        return True
    return (cur_epoch + 1) % cfg.TRAIN.CHECKPOINT_PERIOD == 0 or cur_epoch + 1 == cfg.OPTIMIZER.MAX_EPOCH


def save_checkpoint(path_to_job, model, model_ema, optimizer, epoch, cfg, model_bucket=None, scaler=None):
    """
    Save a checkpoint.
    Args:
...
def checkpoint_preprocess(cfg, model_state):
    ...
    elif cfg.TRAIN.CHECKPOINT_PRE_PROCESS.POS_EMBED == "add-tada-token":
        logger.info("Adding tada token on positional embedding.")
        from models.utils.init_helper import trunc_normal_
        tada_token = trunc_normal_(torch.randn(1, 1, model_state["backbone.pos_embd"].shape[-1]), std=.02)
        model_state["backbone.pos_embd"] = torch.cat(
            (tada_token, model_state["backbone.pos_embd"]), dim=1
        )
    elif cfg.TRAIN.CHECKPOINT_PRE_PROCESS.POS_EMBED == "ignore":
        logger.info("Ignoring positional embedding in checkpoint.")
        if "backbone.pos_embd" in model_state:
            del model_state["backbone.pos_embd"]
        elif 'backbone.pos_embed' in model_state:
            del model_state["backbone.pos_embed"]
    elif cfg.TRAIN.CHECKPOINT_PRE_PROCESS.POS_EMBED is None:
        logger.info("No process on positional embedding.")
    else:
        raise NotImplementedError

    if (hasattr(cfg.VIDEO.BACKBONE, "TUBELET_SIZE") and 
        cfg.VIDEO.BACKBONE.TUBELET_SIZE > 1 and
        cfg.TRAIN.CHECKPOINT_PRE_PROCESS.PATCH_EMBD == "central_frame"):
        ...
        or not hasattr(cfg.VIDEO.BACKBONE, "TUBELET_SIZE")
        or cfg.VIDEO.BACKBONE.TUBELET_SIZE == 1):
        logger.info("No process on patch/tubelet embedding.")
    else:
        raise NotImplementedError

    if cfg.AUDIO.ENABLE and cfg.TRAIN.CHECKPOINT_PRE_PROCESS.POP_SPECTROGRAM_EXTRACTOR:
        logger.info("Poping spectrogram extractor.")
        to_pops = []
        for k in model_state.keys():
...
def load_pre_trained_agent_checkpoint(
    cfg,
    path_to_checkpoint,
    agent,
    data_parallel=True,
    rename=['backbone', 'network.model']
):
    """
    Load the checkpoint from the given file. If inflation is True, inflate the
    2D Conv weights from the checkpoint to 3D Conv.
    ...
    # Account for the DDP wrapper in the multi-gpu setting.
    ms = agent.module if data_parallel else agent
    # Load the checkpoint on CPU to avoid GPU mem spike.
    with open(path_to_checkpoint, "rb") as f:
        checkpoint = torch.load(f, map_location="cpu")
    model_state_dict_3d = (
        agent.module.state_dict() if data_parallel else agent.state_dict()
    )
    checkpoint["model_state"] = normal_to_sub_bn(
        checkpoint["model_state"], model_state_dict_3d
    )
    ...
    for name, p in checkpoint["model_state"].items():
        ...
        fixed_model_state[name] = p

    mismatch = ms.load_state_dict(fixed_model_state, strict=False)
    logger.info("Keys in agent not matched: {}".format(mismatch[0]))
    logger.info("Keys in agent checkpoint not matched: {}".format(mismatch[1]))
    return 
...
def load_test_checkpoint_dqn(cfg, model, model_ema, agent, model_bucket=None):
    """
    Loading checkpoint logic for testing.
    """
    read_from_oss = False
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR)
        load_checkpoint_dqn(cfg, last_checkpoint, model, model_ema, agent, cfg.NUM_GPUS*cfg.NUM_SHARDS > 1, optimizer_model=None, optimizer_agent=None, inflation=False, pre_process=False)
        logger.info("Load from the last checkpoint file: {}".format(last_checkpoint))
    elif cfg.TRAIN.CHECKPOINT_FILE_PATH != "":
        # If no checkpoint found in TEST.CHECKPOINT_FILE_PATH or in the current
        # checkpoint folder, try to load checkpoint from
...
def load_train_checkpoint(cfg, model, model_ema, optimizer, model_bucket=None, scaler=None):
    """
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            cfg, last_checkpoint, model, model_ema, cfg.NUM_GPUS*cfg.NUM_SHARDS > 1, optimizer, scaler=scaler,
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            scaler=None if cfg.TRAIN.FINE_TUNE else scaler,
        ...
        if read_from_oss:
            bu.clear_tmp_file(checkpoint_path)
        if hasattr(cfg.TRAIN, 'AGENT_CHECKPOINT_FILE_PATH') and len(cfg.TRAIN.AGENT_CHECKPOINT_FILE_PATH) > 0:
            _checkpoint_file_path = cfg.TRAIN.AGENT_CHECKPOINT_FILE_PATH
            if _checkpoint_file_path.split(':')[0] == 'oss':
                model_bucket_name = _checkpoint_file_path.split('/')[2]
                if model_bucket is None or model_bucket.bucket_name != model_bucket_name:
                    model_bucket = bu.initialize_bucket(cfg.OSS.KEY, cfg.OSS.SECRET, cfg.OSS.ENDPOINT, model_bucket_name)
                ...
                checkpoint_path = cfg.TRAIN.CHECKPOINT_FILE_PATH
            logger.info("Load from given checkpoint file for agent in models!!\nCheckpoint file path: {}".format(_checkpoint_file_path))
            load_pre_trained_agent_checkpoint(
                cfg,
                checkpoint_path,
                model.module.backbone.patch_selector if cfg.NUM_GPUS*cfg.NUM_SHARDS > 1 else model.backbone.patch_selector,
                False,
