...
def Loss_MaeCls(cfg, preds, logits, labels, cur_epoch=0):
    ...
    preds_ = preds['preds_cls'][0]
    labels_ = labels['supervised']

    # aact---multi-label---BCE
    aact_output = preds_[:, 0: aact_num_classes]
    aact_label_id = labels_[:, 0: aact_num_classes]
    loss1 = nn.BCEWithLogitsLoss(reduction='none')(aact_output.float(), aact_label_id.float())

    # sact---one-label---CE
    sact_output = preds_[:, aact_num_classes: aact_num_classes + sact_num_classes]
    sact_label_id = labels_[:, aact_num_classes: aact_num_classes + sact_num_classes]
    sact_label_id = sact_label_id.argmax(dim=1)
    loss2 = nn.CrossEntropyLoss(reduction='none')(sact_output.float(), sact_label_id)

    # act---one-labe---CE
    act_output = preds_[:, aact_num_classes + sact_num_classes:]
    act_label_id = labels_[:, aact_num_classes + sact_num_classes:]
    act_label_id = act_label_id.argmax(dim=1)
    loss3 = nn.CrossEntropyLoss(reduction='none')(act_output.float(), act_label_id)

    loss_cls = loss1.mean() + loss2.mean() + loss3.mean()
