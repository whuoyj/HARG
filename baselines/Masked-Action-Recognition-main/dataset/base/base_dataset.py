                ...
                with open(local_file) as f:
                    ...
                    for line in lines:
                        for idx in range(self._num_clips):
                            ...
                            self._spatial_temporal_index.append(count)
                            count += 1
    ...
    def _decode_video(self, sample_info, index, num_clips_per_video=1):
        ...
        if self.split == "train" or self.split == "linear_train_cls":
            ...
            if self.cfg.TEST.NUM_SPATIAL_CROPS == 1:
                ...
                self.spatial_idx = self._spatial_temporal_index[index] % self.cfg.TEST.NUM_SPATIAL_CROPS
        # path = "/home/ouyangjun/workspace/data/a/workspace/kinetics-tps/train_videos/0001.mp4"
        frame_list= []
        for idx in range(num_clips_per_video):

            video_name = path.split('/')[-1].split('.')[0]
            # video_name = '20201207231031'
        ...
        x_clamped = torch.clamp(frames, 0, 255)

        frames = x_clamped.to(torch.uint8)
