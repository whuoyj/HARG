...
from .ams_2D_module import CPTM_Bottleneck, MultiScale_Temporal

#from ...utils import get_root_logger
#from ..registry import BACKBONES
#from ..builder import BACKBONES   #new_version
...
class ResNetAMS(nn.Module):
    ...
    def __init__(self,
                 ...
                 temporal_block_indices=(2, ),    #Temporal_Block
                 strides=(1, 2, 2, 2),
                 dilations=(1, 1, 1, 1),
                 style='pytorch',
                 frozen_stages=-1,
        ...
        self._make_temporal_attention_block()
    ...
    def _make_temporal_attention_block(self):

        if self.CompetitiveFusion:
            print("sk only")
            self.multiScale_res2 = CPTM_Bottleneck(256, gamma=self.gamma, segments=self.num_segments)  #tem+sk
            self.multiScale_res4 = CPTM_Bottleneck(1024, gamma=self.gamma, segments=self.num_segments)

        else:
            print("none")
            self.multiScale_res2 = MultiScale_Temporal(256, segments=self.num_segments)   #tem+none
            self.multiScale_res4 = MultiScale_Temporal(1024, segments=self.num_segments)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.temporal_block_indices:
                if i == 0:
                    x = self.multiScale_res2(x)    #res4_CPTM
                elif i == 2:
                    x = self.multiScale_res4(x)    #res2_CPTM
    ...
    def _freeze_stages(self):
        ...
        if self.frozen_stages >= 2:                 #freeze our module param
            for m in self.multiScale.modules():
                for param in m.parameters():
                    param.requires_grad = False
