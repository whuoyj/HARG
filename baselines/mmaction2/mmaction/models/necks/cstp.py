...
from mmengine.model import constant_init, normal_init, xavier_init

#from ..builder import build_loss
#from ..registry import NECKS
#from ..builder import NECKS   #new_version
from mmaction.registry import MODELS
from ..backbones.ams_2D_module import CSTP_Stage1_Adaptive_Fusion, CSTP_Stage2_Adaptive_Fusion
...
class AuxHead(nn.Module):
    ...
    def loss(self, x: torch.Tensor,
        ...
        loss1 = loss(aact_output, aact_label_id)

        # sact---one-label---CE
        sact_output = output[:, aact_num_classes: aact_num_classes + sact_num_classes]
        sact_label_id = label_id[:, aact_num_classes: aact_num_classes + sact_num_classes]
        sact_label_id = sact_label_id.argmax(dim=1)
        loss = nn.CrossEntropyLoss()
        loss2 = loss(sact_output, sact_label_id)

        # act---one-labe---CE
        act_output = output[:, aact_num_classes + sact_num_classes:]
        act_label_id = label_id[:, aact_num_classes + sact_num_classes:]
        act_label_id = act_label_id.argmax(dim=1)
        loss3 = loss(act_output, act_label_id)

        loss_cls = loss1 + loss2 + loss3

        losses['loss_aux'] = self.loss_weight * loss_cls #self.loss_cls(x, labels)
        return losses
...
class CSTP(nn.Module):
    ...
    def __init__(self,
        ...
        self.fusion_top_down = ConvModule(
            out_channels*2,
            out_channels*2,
            1,
            stride=1,
        ...
        self.fusion_botton_up = ConvModule(
            out_channels*2,
            out_channels*2,
            1,
            stride=1,
    ...
    def forward(self, x, data_samples=None):
        ...
        pyramids_top_down, pyramids_botton_up = self.pyramids_selective(Pyramids_inputs)
        top_down_outs = self.fusion_top_down(pyramids_top_down)
        botton_up_outs = self.fusion_botton_up(pyramids_botton_up)

        # fuse two pyramid outs
        '''add-sk-fusion'''
        outs = [top_down_outs, botton_up_outs]
        outs = self.pyramids_fusion(outs)
        outs = self.pyramid_fusion(outs)
